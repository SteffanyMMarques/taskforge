...
void main() {
    IO.println("=== TaskForge v2.2 ===");

    List<Tarefa> listaTarefa = new ArrayList<>();

    listaTarefa.add(new TarefaSimples("Estudar Java", "Estudar Encapsulamento", 1,
            "Lander Andrade", 2, TarefaStatus.PENDENTE));
    listaTarefa.add(new TarefaComPrazo("Estudar Frontend", "Estudar React", 3,
            "Lander Andrade", 2, TarefaStatus.EM_ANDAMENTO, 3));
    listaTarefa.add(new TarefaComPrazo("Estudar Banco de Dados", "Estudar BDD", 3,
            "Lander Andrade", 2, TarefaStatus.PENDENTE, 3));

    for (Tarefa tarefaRecuperada : listaTarefa) {
        tarefaRecuperada.resumo();
    }
